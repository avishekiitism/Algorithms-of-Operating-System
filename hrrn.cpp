...
int checklogic_HRR(){
	...
	for(int i=0;i<vec1.size();i++){
		...
		if( res[res.size()-1].r >= vec1[i].a_t ){
		    int l=res[res.size()-1].r-vec1[i].a_t+vec1[i].b_t;
			float z=(float)l/vec1[i].b_t;
			q.push_back({z,vec1[i].p_no});
		}
	}
	if(q.size()==0)
		return 0;
...
void displayfull(){
	...
	cout<<"Total Turn Around Time: "<<ta_t_avg<<endl;
	cout<<"Total Waiting Time: "<<w_t_avg<<endl;
	ta_t_avg/=vec.size();
	w_t_avg/=vec.size();
	cout<<endl;
...
int main(){
	cout<<"HIGHEST RESPONSE RATIO NEXT SCHEDULING"<<endl<<endl;
	inputData()	;
	solve();
//	display();
	sort_p_no();
