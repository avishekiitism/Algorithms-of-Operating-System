...
void inputData(){
	...
	int q;cin>>q;
	cout<<"Enter the time quanta for each queues (for FCFS enter -1) in priority wise order"<<endl;
	for(int i=0;i<q;i++){
		int a;cin>>a;
		if(a==-1)
			t_q[i]=10000;
...
int checklogic_RBS(){
	...
	for(int i=0;i<vec1.size();i++){
		...
		if( res[res.size()-1].r >= vec1[i].a_t &&  !checkq(vec1[i].p_no) ){	//find(q.begin(), q.end(), vec[i].p_no)==q.end()
			vec1[i].q++;
...
void solve(){
	...
	if(res.size()==0){
		...
		g.l=vec1[0].a_t;
		if( vec1[0].b_t <= t_q[vec1[0].q]){
			g.r=g.l+vec1[0].b_t;
			vec1[0].b_t=0;
		}
		else{
			g.r=g.l+t_q[vec1[0].q];
			vec1[0].b_t=vec1[0].b_t-t_q[vec1[0].q];
		...
		if( x==-1 ){
			g.l=vec[0].a_t;
			if( vec1[0].b_t <= t_q[vec1[0].q-1]){
				g.r=g.l+vec1[0].b_t;
				vec1[0].b_t=0;
			}
			else{
				g.r=g.l+t_q[vec1[0].q-1];
				vec1[0].b_t=vec1[0].b_t-t_q[vec1[0].q-1];
...
// 			vec1[0].q++;
		...
		//	q.push_back(vec1[0].p_no);	
			...
			if( vec1[i].b_t <= t_q[vec1[i].q-1]){
				g.r=g.l+vec1[i].b_t;
				vec1[i].b_t=0;
			}
			else{
				g.r=g.l+t_q[vec1[i].q-1];
				vec1[i].b_t=vec1[i].b_t-t_q[vec1[i].q-1];
...
int main(){
	cout<<"AVISHEK KUMAR\n20JE0221\n\n";
	cout<<"Multilevel Feedback Queue(MLFQ)\n"<<endl<<endl;
	inputData()	;
	solve();
// 	display();
	sort_p_no();
